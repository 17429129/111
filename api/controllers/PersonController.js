...
json: function(req, res) {
    ...
    // funfares function
funfares: function(req, res) {
    Person.find().exec( function(err, persons) {
        return res.view('person/funfares', {'persons': persons});
    });
},
  // view function
...
view: function(req, res) {
  Person.findOne(req.params.id).exec( function(err, model) {
        if (model != null) {
					return res.view('person/view', {'persons': model});
...
admin: function(req, res) {
	...
	Person.find().paginate({page:req.query.page,limit:5})
	.exec( function(err, persons) {
		 Person.count().exec(function(err,value){
				var pages=Math.ceil(value/5);
    Person.find().exec( function(err, persons) {
        return res.view('person/admin', {'persons': persons,'count':pages});
    });
	});
});
},
